...
class nats:
    ...
    def monitor_untracked_info(self, info: dict, module_name="UNKNOWN"):
        ...
        """
        if self.monitor:
            asyncio.create_task(self.monitor.monitor(info, module_name=module_name))
        else:
            LOGGER.warning(f"Untracked info: {info} not sent to monitor")

    def monitor_file(self, variable_name: str, file_path: str, module_name: str):
        """
        This method monitors a file and sends its content to the monitor.
        ...
        if self.monitor:
            ...
            LOGGER.warning(
                f"Untracked file: {file_path} not sent to monitor for variable {variable_name}. Monitor not set."
            )
