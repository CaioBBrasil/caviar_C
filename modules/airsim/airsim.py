...
class airsim(module):
    ...
    def _do_init(self):
        ...
        command = [
            beach_street_path,
            "-WINDOWED",  # Comment this line if you want to see the AirSim window
            "-ResX=1",
            "-ResY=1",
            "-RenderOffscreen",
        ...
        ffmpeg_command = [
            ...
            "-",
            "-filter:v",
            "minterpolate=fps=15:mi_mode=mci",  # Frame interpolation filter
            "-c:v",
            "libx264",
            "-preset",
    ...
    async def _execute_step(self):
        ...
        if ((current_epoch - self.ini_epoch) / 1e6) < 125:  # 500 ms
            return
    ...
    async def __start_video_streaming(self):
        ...
        Frame interpolation is handled by FFmpeg filter.
        """
        while True:
            ...
            await asyncio.sleep(1 / 8)  # Sleep according to actual FPS (8 FPS)
