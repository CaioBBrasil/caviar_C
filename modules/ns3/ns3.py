...
class ns3(module):
    def _do_init(self):
        ...
        config_command = [
            ...
            "configure",
            "--enable-examples",
            "--build-profile=debug",  # change optimized to debug in case some error happens;
            "--enable-eigen",
            "--disable-werror",
            "--enable-sudo",
        ]
        ...
        run_command = ["./ns3", "run", "--enable-sudo", "scratch-simulator"]
    ...
    def create_msg_fifo(self, information):
    ...
    async def _execute_step(self):
        ...
        message_req = list(self.buffers.values())[0].get()[0][1]["message"]

        #print(f"debuc msg sync recived  NS3 \n{message_req}\n")
        ...
        with open(os.path.join(self.ns3_path, "mobility.fifo"), "w") as f:
            f.write(json.dumps(message_req) + "\n")
        time.sleep(0.1)
